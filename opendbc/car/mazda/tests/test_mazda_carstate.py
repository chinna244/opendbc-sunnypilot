...
@pytest.mark.parametrize("alpha_long", [False, True])
def test_startup_held_tja_is_not_a_rising_edge(alpha_long):
  CI, _ = _ci(alpha_long)
...
@pytest.mark.parametrize("alpha_long", [False, True])
def test_per_edge_snapshot_classification(alpha_long):
  CI, _ = _ci(alpha_long)
  p = CANPacker("mazda_2017")
...
@pytest.mark.parametrize("alpha_long", [False, True])
def test_rapid_second_tja_keeps_confident_snapshot(alpha_long):
  CI, _ = _ci(alpha_long)
  p = CANPacker("mazda_2017")
  _arm_tja_sm(CI, p, alpha_long, OFF)
  _update(CI, p, alpha_long, {"TJA_BUTTON": 1}, OFF)
...
@pytest.mark.parametrize("alpha_long", [False, True])
def test_reset_tja_fail_closed(alpha_long):
  CI, _ = _ci(alpha_long)
  p = CANPacker("mazda_2017")
  _arm_tja_sm(CI, p, alpha_long, OFF)
  _update(CI, p, alpha_long, {"TJA_BUTTON": 1}, OFF)
